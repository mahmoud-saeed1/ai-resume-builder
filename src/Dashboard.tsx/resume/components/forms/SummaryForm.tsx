...
const SummaryForm = React.memo(({ enableNextBtn, handleEnableNextBtn, handleDisableNextBtn }: IFormProbs) => {
  /*~~~~~~~~$ States $~~~~~~~~*/
  ...
  }, [setResumeInfo, clearErrors, setValue, handleDisableNextBtn]);

  const handleOnSubmit: SubmitHandler<{ summary: string }> = useCallback(async (data) => {
    setIsLoading(true);
    if (!params?.resumeId) {
  ...
  }, [params.resumeId, handleEnableNextBtn]);

  const handleGenerateSummary = useCallback(async () => {
    setIsGenerated(true);
    const jobTitle = resumeInfo?.personalData?.[0]?.jobTitle || "your job title";
  ...
  const handleSummaryClick = useCallback((summaryText: string) => {
    setValue("summary", summaryText); // Update the form value
    textAreaRef.current!.value = summaryText; // Directly update the text area
    setResumeInfo((prev) => ({
      ...prev,
      summary: summaryText,
    }));
    clearErrors("summary");
    textAreaRef.current?.focus(); // Focus on the text area after selection
  }, [setValue, setResumeInfo, clearErrors]);
    ...
    <FormContainer className="space-y-4" enableNextBtn={enableNextBtn} handleOnSubmit={handleSubmit(handleOnSubmit)} isLoading={isLoading} formTitle="Summary">
      ...
      <Form onSubmit={handleSubmit(handleOnSubmit)}>
        <FormTextarea
          ...
          ref={textAreaRef}  // Ensure ref is passed correctly
          className=""
        />
      </Form>
    </FormContainer>
...
//     // Update the text area with the selected summary
//     setValue("summary", summaryText); // Update the form value
//     textAreaRef.current!.value = summaryText; // Directly update the text area
//     setResumeInfo((prev) => ({
//       ...prev,
